...
async def process_batch(batch, index, line_offsets, corpus_path, embedding_model, gemini_model, existing_answers):
    ...
    for i, query_embedding in enumerate(query_embeddings):
        ...
        Select useful part to the Question from the Contexts.
        Get the most relevant part from the Contexts that can answer the Question.
        If you think none of the Contexts are useful to the Question, say "Generate with your knowledge". 
        DO NOT SUMMARIZE.

        Question: {questions[i]}

        Contexts: {" ".join(relevant_snippets)}

        Relevant Sentences:
        '''
        gemini_prompts.append(gemini_prompt)

    extracted_snippets = await call_gemini(gemini_prompts, gemini_model)

    answer_prompts = []
    for i, snippets in enumerate(extracted_snippets):
        prompt = f'''
    ...
    predictions = await call_gemini(answer_prompts, gemini_model)
    ...
    for i, prediction in enumerate(predictions):
        ...
        print("-"*50)
        print(f"What first Gemini selected: {extracted_snippets[i]}")
        print("-"*50)
        print(f"Final answer from second Gemini: {prediction}")
        print("="*50)
